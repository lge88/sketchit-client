(function() {
	...
	if(window.Ext) {
		...
		//TODO: implement deep copy object
		ut.copy = ut.apply;
	} else {
	...
	ut.Cmpnt = ut.extend(Object, {
		...
		resetToSystemDefaults : function() {
			ut.copy(this.defaults, this.systemDefaults);
		},
		batchEdit:function(key,value){
			var batch;
			batch=new ut.BasicMove("set",{
				addr:this.data,
				key:key,
				value:value								
			});
	...
	ut.Collection = ut.extend(Array, {
		constructor : function(m) {
			this.model = ut.isDefined(m) ? m : ut.Cmpnt;
			//alert("here")
		},
		setModel : function(m) {
			this.model = m;
