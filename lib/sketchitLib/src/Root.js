	...
	Root = Ext.extend(lib.Undoable, {
		...
		commands : {
			...
			batchGenerator : {
				...
				addALineElement : function(args) {
					...
					//if(Ext.isDefined(this.options.pointSnapThreshold) && this.options.pointSnapThreshold > 0) {
					...
					//if(Ext.isDefined(this.options.lineSnapThreshold) && this.options.lineSnapThreshold > 0) {
					...
					//if(Ext.isDefined(this.options.grid)) {
					...
					for(i in this.data.nodes.data) {
						if(this.data.nodes.data.hasOwnProperty(i)) {
							...
							if(dotLineLength(this.data.nodes.data[i].data.X, this.data.nodes.data[i].data.Y, p[0].data.X, p[0].data.Y, p[1].data.X, p[1].data.Y, true) < 0.5) {
				...
				addASPConstraint : function(args) {
					var batch, p, np, newFlag, spc, angle, dir, n;
					batch = [];
					newFlag = true;
					p = new lib.Node({
						data : {
					...
					if(this.options.SPCSnapToDirection) {
						...
						if(Math.abs(args.angle - n * (Math.PI / 2)) / (Math.PI / 2) < this.options.SPCSnapThreshold) {
							angle = n * (Math.PI / 2);
							switch ((n+4)%4) {
								case 0:
									dir = "left";
					...
					Ext.each(this.data.SPConstraints.data, function(spc, i, ar) {
						...
						if(d < this.options.pointSnapThreshold) {
							if(pin) {
								...
								if(spc.data.direction === "up" || pc.data.direction === "down") {
									if(spc.data.X !== 0) {
										batch.push({
											action : "set",
											selector : ["SPConstraints", i],
								...
								} else if(spc.data.direction === "left" || pc.data.direction === "right") {
									if(spc.data.Y !== 0) {
										batch.push({
											action : "set",
											selector : ["SPConstraints", i],
		...
		doHandler : function(command) {
			...
			if(this.validateBatch(batch)) {
				...
				console.log("invalid command args for command " + command.name)
				return false;
			}
