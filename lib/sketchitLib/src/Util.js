(function() {
	...
	ObjectArrayStore = ut.inherit(ArrayWithStrongType, Undoable, {
		...
		//default ->return max value; 0->return max index; 1->return object
		getMax: function(propName,mode){
			//mode
			var v=this[0][propName],i=0;
			while(i+1<this.max+1 && this[i+1][propName]>v){
				v=this[i+1][propName];
				i=i+1;
			}
			if (mode==0){
		...
		//default ->return abs max value; 0->return max index; 1->return object
		getAbsMax: function(propName,mode){
			//mode
			console.log("propName ",propName)
			console.log("this 0",this[0])
			console.log("this 0 v",this[0][propName])
			var v=Math.abs(this[0][propName]),i=0;
			while(i+1<this.max+1 && Math.abs(this[i+1][propName])>v){
				v=Math.abs(this[i+1][propName]);
				i=i+1;
			}
			console.log("v ",v)
			if (mode==0){
