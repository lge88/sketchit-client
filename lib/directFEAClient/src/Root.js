(function() {
	...
	Root = Ext.extend(lib.Undoable, {
		...
		data:{
			...
			nodes:undefined,
			masses:undefined,
			SPConstraints:undefined,
			geomTransforms:undefined,
			materials:undefined,
			sections:undefined,
			elements:undefined,
			patterns:undefined,
		...
		options:undefined,//defined in controller
		...
		initList : function() {
			...
			this.data = {
				...
				materials : new lib.UndoableList({
					model : "Material",
					name : "materials",
					data : []
				}),
			...
			this.undoStack = [];
			this.redoStack = [];
			//window.xx=new sketchitLib.TimeSeries();
			//window.xx1=new sketchitLib.TimeSeries({defaults:{subclass:"Path"}});
			//window.xx2new=new sketchitLib.TimeSeries({defaults:{subclass:"Constant"}});
		},
		commands : {
