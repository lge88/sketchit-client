	...
	Root = ut.extend(ut.Undoable, {
		...
		constructor : function() {
			...
			ut.apply(this.options, {
				...
				Node : ut.extend(ut.Node, {
					shapeConstructor : this.renderer.options.NodeShape
				}),
				defaultLineElementType : "ElasticBeamColumn",
				ElasticBeamColumn : ut.extend(ut.ElasticBeamColumn, {
					shapeConstructor : this.renderer.options.ElementShape;
				}),

			})
			ut.apply(this, {
				nodes : new ut.NodesStore({
					model : this.options.Node
				}),
				elements : new ut.ElementsStore({
					model : ut.Elemnet,
					defaultLineElementType : this.options[this.options.defaultLineElementType]
				})
		...
		batchSplitLineElement : function(args) {
			var Element = args.element.constructor, ne, batch;
			ne = new Element({
				from : args.node,
				to : args.element.to
			}), batch;
		...
		batchAddALineElement : function(args) {
			...
			Element = ut.isDefined(args.type) ? this.options[this.options.defaultLineElementType] : this.options[args.type];
			batch = new ut.Batch();
			n = [];
			temp = [];
			nid = [-1, -1];
		...
		batchCommands : {
			...
			"addALineElement" : function() {
				if(arguments.length === 1 && ut.isObject(arguments[0])) {
					return this.nodes.batchAddALineElement.call(this, arguments[0]);
				}
			},
