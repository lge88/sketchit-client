(function() {
	...
	BatchRunnable = ut.extend(Runnable, {
		run : function(command) {
			if(ut.isDefined(this.commands[command.name])) {
				...
				var result;
				result = this.batchCommands[command.name].call(this, command.args);
				if(result != false) {
	...
	ScriptRunnable = ut.extend(Undoable, {
		...
		run : function() {
			...
			inlineRun = function() {
				...
				if(ut.isDefined(this.commands[command])) {
					...
					if(result != false) {
						//console.log("is bsm",result.batch instanceof BasicMove)
						//console.log("is batch",result.batch instanceof Batch)
	...
	ObjectArrayStore = ut.inherit(ArrayWithStrongType, ScriptRunnable, {
		count : 0,
		max : -1,
		startId : 0,
		...
		batchRemoveAt : function(index) {
			...
			var value, bm = new Batch(), max = this.max, count = this.count;
			if (index==-1){
				index=max;
			}
