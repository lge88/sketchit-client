		...
		constructor : function() {
			...
			ut.apply(this.batchCommands, {
				...
				"addALoad" : function() {
					...
					var Node, nLoad, ePLoad, uLoad, n, l, nid, lid, i, temp, splitOperation = [];
					Node = lib.Node;
					//Element = type;
					nLoad = lib.NodeLoad;
				...
				"loadResultData" : function(str) {
					...
					while(i < len) {
						if(ut.isDefined(this.theNodes[index])) {
							this.theNodes[index].dispX = arr[i++];
							this.theNodes[index].dispY = arr[i++];
							this.theNodes[index].dispRZ = arr[i++];
							index++;
						} else {
							index++;
						}
