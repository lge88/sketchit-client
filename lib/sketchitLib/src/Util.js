(function() {
	...
	stroke = ut.extend(Shape, {
		...
		display : function() {
			this.ctx.save();
			this.ctx.transform.apply(this.ctx, this.transform);
			this.ctx.lineWidth = this.lineWidth;
			this.ctx.strokeStyle = "rgba(" + this.strokeStyle[0] + "," + this.strokeStyle[1] + "," + this.strokeStyle[2] + "," + this.strokeStyle[3]/255 + ")";
			this.ctx.beginPath();
	...
	filled = ut.extend(Shape, {
		...
		display : function() {
			...
			this.ctx.strokeStyle = "rgba(" + this.edgeStrokeStyle[0] + "," + this.edgeStrokeStyle[1] + "," + this.edgeStrokeStyle[2] + "," + this.edgeStrokeStyle[3]/255 + ")";
			this.ctx.fillStyle = "rgba(" + this.fillStyle[0] + "," + this.fillStyle[1] + "," + this.fillStyle[2] + "," + this.fillStyle[3]/255 + ")";
			
			this.ctx.beginPath();
			this.ctx.moveTo(this.vertice[0], this.vertice[1]);
			for( i = 0; i < len - 1; i++) {
				//this.ctx.strokeStyle="rgb(255,0,"+50*i+")";
				this.ctx.lineTo(this.vertice[2 * i + 2], this.vertice[2 * i + 3]);
			}
			this.ctx.closePath();
			this.ctx.fill();
			if(this.showEdge) {
				//this.ctx.strokeStyle="rgb(255,0,"+130+")";
				this.ctx.stroke();
			}
			this.ctx.fill();
			this.ctx.restore();
	...
	ObjectArrayStore = ut.inherit(ArrayWithStrongType, Undoable, {
		...
		constructor : function() {
			//Runnable.superclass.constructor.apply(this, arguments);
